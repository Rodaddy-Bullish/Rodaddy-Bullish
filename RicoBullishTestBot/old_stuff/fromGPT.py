...
async def tlg_connect (api_id, api_hash, phone_number, session_string=None):
...
async def download_all_media(client, messages, folder_path, chat_name):
    ...
    for message in tqdm(messages):
        if message.media:
            await message.download_media(f'files/{chat_name}/')
...
async def main ():
    ...
    client = await tlg_connect(env_vars["app_id"],
                               env_vars['app_hash'],
                               env_vars['phone_num'],
    ...
    for dialog in dialogs:
        ...
        if dialog.is_group or dialog.is_channel:
            ...
            if messages:
                ...
                # download_media(messages, client, chat_name, base_folder_path='telegram_messages')
                for message in messages:
